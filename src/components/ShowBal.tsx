...
const ShowBal = () => {
    const { setTotalIncome, totalIncome , savings, setSavings, target} = useContext(
      BalanceContext
    ) as IContext;
    ...
    return (
        <div >
            ...
            <div>
                ...
                <input type="number"  className='transfer' onChange={(e)=> setValue(e.target.value)} value={value}/> <button onClick={handleClick}>Transfer</button>
            </div>
    </div>
  )
}
